...
public class NewsSvc {
	...
	public List<News> listDgNews(String title, int page, int rows) {
		if (title == null || "".equals(title))
			return dao.find("from News news order by news.tjdate desc",
					new Object[0], page, rows);
		else
			return dao
					.find("from News news WHERE news.title like ? order by news.tjdate desc",
							new Object[] { '%' + title + '%' }, page, rows);
	}
